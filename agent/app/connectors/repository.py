"""
Postgres persistence for all connector snapshots (runs, per-service rows, aggregate
signals, topology edges, and normalized log event summaries).
"""
...
log_events_t = Table(
    ...
    Column("service_name", String, nullable=False),
    Column("namespace", String, nullable=True),
    Column("pod", String, nullable=True),
...
def _insert_log_events(conn: Connection, run_id: uuid.UUID, logs: Dict[str, Any]) -> None:
    for event in (logs.get("events") or []):
        conn.execute(
            insert(log_events_t).values(
                run_id=run_id,
                ...
                message_sample=event.get("message_sample"),
            )
        )


def _delete_snapshot_children(conn: Connection, run_id: uuid.UUID) -> None:
    conn.execute(delete(service_metrics_t).where(service_metrics_t.c.run_id == run_id))
    conn.execute(delete(signals_t).where(signals_t.c.run_id == run_id))
    conn.execute(delete(topology_t).where(topology_t.c.run_id == run_id))
    conn.execute(delete(log_events_t).where(log_events_t.c.run_id == run_id))


def _insert_snapshot_children(conn: Connection, run_id: uuid.UUID, data: Dict[str, Any]) -> None:
    ...
    _insert_log_events(conn, run_id, data.get("logs") or {})
...
def create_run(conn: Connection, data: Dict[str, Any] | None = None) -> uuid.UUID:
    """
    Create an ingestion run before connector collection starts.

    The orchestrator uses this to establish a shared ``run_id``; connector
    workers then replace/update the snapshot for the same run.
    """
    run_id = uuid.uuid4()
    snapshot = data or _empty_snapshot()
    data_quality = snapshot.get("data_quality") or {}
    conn.execute(insert(runs_t).values(id=run_id, snapshot=snapshot, data_quality=data_quality))
    _insert_snapshot_children(conn, run_id, snapshot)
    return run_id


def delete_run(conn: Connection, run_id: uuid.UUID) -> None:
    """Delete one run and its queryable child rows."""
    _delete_snapshot_children(conn, run_id)
    conn.execute(delete(runs_t).where(runs_t.c.id == run_id))


def save_run(conn: Connection, data: Dict[str, Any]) -> uuid.UUID:
    """
    Insert one complete run and child rows. ``data`` matches ``normalize`` output
    plus optional ``topology`` override.
    """
    return create_run(conn, data)


def load_run_snapshot(conn: Connection, run_id: uuid.UUID) -> Dict[str, Any] | None:
    """Return one run snapshot JSONB, or None when the run does not exist."""
    snapshot = conn.execute(select(runs_t.c.snapshot).where(runs_t.c.id == run_id)).scalar_one_or_none()
...
def load_latest_snapshot(conn: Connection) -> Tuple[uuid.UUID, Dict[str, Any]] | None:
    """Return the latest run id and full snapshot JSONB, if any."""
    row = conn.execute(
        select(runs_t.c.id, runs_t.c.snapshot).order_by(desc(runs_t.c.created_at)).limit(1)
    ).mappings().first()
    if not row:
        return None
...
def load_run_as_raw_state(
    conn: Connection,
    run_id: Optional[uuid.UUID],
) -> Tuple[Dict[str, float], Dict[str, Any], Dict[str, Any], uuid.UUID]:
    """
    Build ``raw_signals``, ``raw_topology``, and ``raw_logs`` for ``GraphState`` from stored rows.
    ...
    snapshot = conn.execute(select(runs_t.c.snapshot).where(runs_t.c.id == rid)).scalar_one_or_none() or {}
    ...
    log_signals = ((snapshot.get("logs") or {}).get("signals") or {})
    if isinstance(log_signals, dict):
        for key, val in log_signals.items():
            if val is None:
                continue
            if isinstance(val, (int, float, bool)):
                raw_signals.setdefault(key, float(val))
    ...
    log_service_signals = ((snapshot.get("logs") or {}).get("service_signals") or {})
    if isinstance(log_service_signals, dict):
        for service, signals in log_service_signals.items():
            if service in service_details and isinstance(signals, dict):
                service_details[service]["log_summary"] = signals
    ...
    return raw_signals, raw_topology, raw_logs, rid
